  ...
  const value = useMemo(() => {
    ...
    const updateBookingStatus = (bookingId, status, providerEmail) => {
      setPortalData((prev) => {
        ...
        const shouldSeedThread = normalizedStatus === "ACCEPTED" && existingThread.length === 0;
              ...
              [bookingId]: [
                {
                  id: createMessageId(),
                  from: "System",
                  text: "Booking accepted. Chat is now enabled for this booking.",
    ...
    const addBookingMessage = (message) => {
      ...
      setPortalData((prev) => {
        ...
        if (booking.status !== "ACCEPTED") return prev;
