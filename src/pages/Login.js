...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const authenticatedRole = resolveRole(getRoleFromAuthResponse(res?.data) || role) || role;
      ...
      if (!res?.data?.token) {
        ...
        const hasFallbackIdentity = Boolean(
          res?.data?.email || res?.data?.user?.email || res?.data?.name || res?.data?.user?.name
        );
        const fallbackEmail =
          normalizeEmail(res?.data?.email || res?.data?.user?.email || normalizedEmail);
        const fallbackName =
          res?.data?.name || res?.data?.user?.name || fallbackEmail.split("@")[0];
      ...
      if (
        !persistAccountRole({
          ...
          accountName: res.data?.name || normalizedEmail.split("@")[0],
          assignedRole: authenticatedRole
        })
      ) {
        return;
      ...
      login({ ...res.data, role: authenticatedRole });
      redirectByRole(authenticatedRole);
    } catch (err) {
      if (err?.response) {
        if (loginAsOpenAdmin()) return;
